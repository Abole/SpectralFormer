...
数据使用Matlab R2016a及以上版本生成，网络代码在Ubuntu系统的Python 3.7下使用PyTorch 1.6版本(CUDA 10.1)进行测试。
...
该工具箱由两个提出的模块组成，即群智能光谱嵌入(GSE:通过设置band_patches大于1)和跨层自适应融合(CAF:通过设置mode为CAF)，可以即插即用地进行逐像素和逐补丁的高光谱图像分类。有关详情，请参阅有关文件。
本文以**印第安松高光谱数据**为例进行了实验。直接运行带有不同网络参数设置的**demo.py**函数来产生结果。请注意，由于参数初始化的随机性，实验结果可能与论文报道略有不同。
请小心分配参数，如' epoches '和' weight_decay '。为了重新执行本文表八中的最后三列，即**ViT**， **像素**和**Patch-wise SpectralFormer**在Indian Pines数据集中，您可以通过以下方式重新训练:
...
如果在Pavia和Houston数据集上测试补丁型SpectralFormer时遇到内存错误，注释trainloader和其他一些不必要的代码部分或将整个图像分成几个子图像都可以工作。 如果您希望在自己的数据中运行代码，则可以相应地更改输入(例如，数据、标签)并调整参数。 如果您在使用此代码时遇到错误，请不要犹豫与我们联系。
